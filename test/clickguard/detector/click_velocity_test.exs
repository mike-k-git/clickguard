defmodule Clickguard.Detector.ClickVelocityTest do
  ...
  describe "detect/2" do
    test "6 clicks at ~600ms median → :high" do
      events = EB.burst({127, 0, 0, 1}, @base_ts, 6, 600, user_agent: "test-velocity-ua")
    ...
    test "6 clicks at ~1500ms median → :medium" do
      events = EB.burst({127, 0, 0, 1}, @base_ts, 6, 1500, user_agent: "test-velocity-ua")
    ...
    test "6 clicks at ~10s median → no finding" do
      events = EB.burst({127, 0, 0, 1}, @base_ts, 6, 10_000, user_agent: "test-velocity-ua")

      [] = ClickVelocity.detect(events, [])
    end

    test "4 clicks at 500ms → no finding (min-size gate)" do
      events = EB.burst({127, 0, 0, 1}, @base_ts, 4, 500, user_agent: "test-velocity-ua")
    ...
    test "two 4-click runs separated by 40 min → no finding" do
      events =
        EB.burst({127, 0, 0, 1}, @base_ts, 4, 500, user_agent: "test-velocity-ua") ++
          EB.burst({127, 0, 0, 1}, DateTime.add(@base_ts, 40, :minute), 4, 500,
            user_agent: "test-velocity-ua"
          )
    ...
    test "5 clicks inside one second within an otherwise slow 10-click session → :high via burst" do
      events =
        EB.burst({127, 0, 0, 1}, @base_ts, 5, 15_000, user_agent: "test-velocity-ua") ++
    ...
    test "one pair, two offending sessions → ONE finding, event_count = sum, severity = worst" do
      events =
        EB.burst({127, 0, 0, 1}, @base_ts, 10, 500, user_agent: "test-velocity-ua") ++
    ...
    test "jitter intervals" do
      events = [
        EB.event({127, 0, 0, 1}, @base_ts, user_agent: "test-velocity-ua"),
    ...
    test "sample_events contains events from the worst session" do
      worst = EB.burst({127, 0, 0, 1}, @base_ts, 6, 50, user_agent: "test-velocity-ua")

      bad =
        EB.burst({127, 0, 0, 1}, DateTime.add(@base_ts, 35, :minute), 6, 500,
          user_agent: "test-velocity-ua"
        )
    ...
    test "sample_events contains events from the worst session #2" do
      ...
      bad =
        EB.burst({127, 0, 0, 1}, DateTime.add(@base_ts, 35, :minute), 6, 1_500,
          user_agent: "test-velocity-ua"
        )
